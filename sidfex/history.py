...
#first scan, ensure getting all of the older days:
        #if (obs_year < 2020. and (obs_date >= dawn) ): 
#More normal scan
        if ((obs_date >= dawn) ): # and (obs_date-dawn)/freq % = 0
          obs_lat    = float(words[6])
          obs_lon    = float(words[7])
          #print(obs_date, " ll = ",obs_lat, obs_lon, (obs_date-dawn)/freq )
