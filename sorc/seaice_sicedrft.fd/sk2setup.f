...
COLD 2000 CONTINUE
COLD        READ (48, 9001, ERR=2009, END=2010) xi(i), yi(i), ew(i),
COLD     1    xi(i+1), yi(i+1), ew(i+1),
COLD     2    xi(i+2), yi(i+2), ew(i+2),
COLD     3    xi(i+3), yi(i+3), ew(i+3)
COLD        i = i + 4
COLD        GO TO 2000
COLD 2009 CONTINUE
COLD      PRINT *,'Reading error, in north ice line, i = ',i
COLD 2010 CONTINUE
COLD      nuse = i - 1
COLD 9001 FORMAT (4(G3.0,X,G4.0,A1,X))
COLD
COLD      j = nuse
COLDC      DO 2020 i = 1, j
COLD      DO 2020 i = 2, j
COLD        xi(i) = xi(i)/10.
COLD        yi(i) = yi(i)/10.
COLD        IF (ew(i) .EQ. 'W') THEN
COLD          yi(i) = 360.-yi(i)
COLD        ENDIF
COLD        IF (ABS(xi(i)) .GT. 90.) THEN
COLD          nuse = i-1
COLD          GO TO 2030
COLD        ENDIF
COLD        IF (ABS(yi(i)) .GT. 360.) THEN
COLD          nuse = i-1
COLD          GO TO 2030
COLD        ENDIF
COLD 2020 CONTINUE
COLD 2030 CONTINUE
COLD      PRINT *,'northern points = ',nuse

COLDC     Now read in the southern ice line
COLD      i = nuse+1
COLD      READ (49, 9002) header
COLD      PRINT *,'header south ',header
COLD 2100 CONTINUE
COLD        READ (49, 9001, ERR=2109, END=2110) xi(i), yi(i), ew(i),
COLD     1    xi(i+1), yi(i+1), ew(i+1),
COLD     2    xi(i+2), yi(i+2), ew(i+2),
COLD     3    xi(i+3), yi(i+3), ew(i+3)
COLD        i = i + 4
COLD        GO TO 2100
COLD 2109 CONTINUE
COLD      PRINT *,'Reading error, in south ice line, i = ',i
COLD 2110 CONTINUE
COLD      nuses = i - 1
COLD
COLD      DO 2120 i = nuse+1, nuses
COLD        xi(i) = -xi(i)/10.
COLD        yi(i) =  yi(i)/10.
COLD        IF (ew(i) .EQ. 'W') THEN
COLD          yi(i) = 360.-yi(i)
...
COLD 2120 CONTINUE
COLD 2130 CONTINUE
COLD      PRINT *,'Southern points = ',nuses
