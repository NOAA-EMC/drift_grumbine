...
!     Robert Grumbine 9 December 1992 
...
!     Declare passing argument array variables.
...
!     Operational code.
!     Calls to crho, cf, and uageo moved to atmos. 3/23/92.

!     Compute the gridded ice drift velocity field
...
!     Compute the velocity of the floes by bilinear interpolation.
      CALL flovel(ui, vi, nlong, nlat, uf, vf, eta, lambda, ig, jg, npts) 

!     Now are ready to extrapolate the position of the floe.
      DO k = 1, npts
        IF (ABS(y(k)) .LT. 89.5) THEN !COS is < 0.01 poleward of this
          dx(k) = dx(k) + dt*uf(k)/radius/rdpdg/COS(rdpdg*y(k))
!         Avoid multiple wrap-arounds when near, but not at pole
          dx(k) = AMOD(dx(k), 360.)
         ELSE
...
!     Bullet-proofing: verify that extrapolated positions lie
!       within the domain.
!     Note that the new dx values must also be adjusted.
      DO k = 1, npts
 9200   CONTINUE
