...
!       lies in the bounds of the four corner points.
      DO k = 1, npts
        uimx = AMAX1(ui(ig(k)  , jg(k)  ), ui(ig(k)+1, jg(k)  ),   &
                     ui(ig(k)  , jg(k)+1), ui(ig(k)+1, jg(k)+1)  ) 
        uimn = AMIN1(ui(ig(k)  , jg(k)  ), ui(ig(k)+1, jg(k)  ),   &
                     ui(ig(k)  , jg(k)+1), ui(ig(k)+1, jg(k)+1)  ) 
        vimx = AMAX1(vi(ig(k)  , jg(k)  ), vi(ig(k)+1, jg(k)  ),   &
                     vi(ig(k)  , jg(k)+1), vi(ig(k)+1, jg(k)+1)  ) 
        vimn = AMIN1(vi(ig(k)  , jg(k)  ), vi(ig(k)+1, jg(k)  ),   &
                     vi(ig(k)  , jg(k)+1), vi(ig(k)+1, jg(k)+1)  ) 
